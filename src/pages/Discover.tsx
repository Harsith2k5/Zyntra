...
interface OCMData {
  ...
  address?: { line1?: string; line2?: string; town?: string; state?: string; postcode?: string; country?: string; };
  location?: { lat?: number; lng?: number; };
  operator?: string; status?: string;
  connections?: Array<{ type?: string; power_kw?: number; voltage?: number; amps?: number; level?: string; current?: string; }>;
  pricing?: string; usage_type?: string; number_of_points?: number; date_created?: string;
  date_last_verified?: string; data_provider?: string; error?: string;
}
...
const MapWithChargingStations: React.FC = () => {
  ...
  useEffect(() => {
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        ...
        () => {
          setUserLocation(defaultHyderabadLocation);
          setLoading(false);
        }
      );
    } else {
  ...
  useEffect(() => {
    const fetchUserEVData = async (user: any) => {
      ...
      try {
        ...
        console.error("Error fetching user EV data:", error);
        setAiLoading(false);
      }
    };
    const unsubscribe = auth.onAuthStateChanged(user => {
      if (user) {
        fetchUserEVData(user);
      } else {
        setAiLoading(false);
      }
    });
  ...
  const findNearbyStations = useCallback((mapInstance: google.maps.Map, location: { lat: number; lng: number }) => {
    ...
    service.nearbySearch({ location: new google.maps.LatLng(location.lat, location.lng), radius: 5000, keyword: "EV charging station" }, (results, status) => {
      if (status === google.maps.places.PlacesServiceStatus.OK && results) {
        const fetchedStations = results.map(r => ({ place_id: r.place_id || '', name: r.name || '', geometry: { location: { lat: () => r.geometry?.location?.lat() || 0, lng: () => r.geometry?.location?.lng() || 0 } }, formatted_address: r.formatted_address, rating: r.rating, user_ratings_total: r.user_ratings_total, opening_hours: r.opening_hours, formatted_phone_number: r.formatted_phone_number } as Station));
        setStations(fetchedStations);
        if (userLocation) {
          const stationsWithDistance = fetchedStations.map(s => ({ ...s, distance: calculateDistance(userLocation.lat, userLocation.lng, s.geometry.location.lat(), s.geometry.location.lng()) }));
          const sortedStations = [...stationsWithDistance].sort((a, b) => a.distance - b.distance);
          setTopStations(sortedStations.slice(0, 3));
          setShowTopStations(true);
        }
      }
    });
  ...
  const showStationDetails = async (station: Station): Promise<void> => {
    setSelectedStation(station);
    setDirections(null);
    const generateMockOCMData = (stationName: string): OCMData => {
      const mockConnections = [[{ type: "CCS Combo 2", power_kw: 50 }, { type: "Type 2", power_kw: 22 }], [{ type: "Type 2", power_kw: 7.4 }, { type: "Bharat AC-001", power_kw: 10 }], [{ type: "CCS Combo 2", power_kw: 120 }, { type: "CHAdeMO", power_kw: 50 }], [{ type: "CCS Combo 2", power_kw: 25 }, { type: "Type 2", power_kw: 7.2 }], [{ type: "Type 2", power_kw: 3.3 }, { type: "16A Socket", power_kw: 3 }]];
      const mockSet = mockConnections[Math.floor(Math.random() * mockConnections.length)];
      return { operator: stationName, status: "Operational", connections: mockSet, pricing: `₹${(18 + Math.random() * 5).toFixed(2)}/kWh`, data_provider: "Zyntra Mock Data" };
    };
    const isDataIncomplete = (data: OCMData): boolean => !data.connections || data.connections.length === 0 || (data.connections[0] && data.connections[0].type?.includes("Type ID") && !data.connections[0].power_kw) || !!data.operator?.toLowerCase().includes("unknown");
    try {
      ...
      if (data.error || isDataIncomplete(data)) {
        setSelectedStation(prev => ({ ...prev, ocmData: generateMockOCMData(station.name) } as StationWithOCM));
      } else { setSelectedStation(prev => ({ ...prev, ocmData: data } as StationWithOCM)); }
    } catch (error) {
      setSelectedStation(prev => ({ ...prev, ocmData: generateMockOCMData(station.name) } as StationWithOCM));
    }
    if (!userLocation || !map || !window.google?.maps?.DirectionsService) return;
    const directionsService = new google.maps.DirectionsService();
    directionsService.route({ origin: new google.maps.LatLng(userLocation.lat, userLocation.lng), destination: new google.maps.LatLng(station.geometry.location.lat(), station.geometry.location.lng()), travelMode: google.maps.TravelMode.DRIVING }, (result, status) => {
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', fontFamily: 'Orbitron, sans-serif', background: primaryBg, color: textColor }}>
      <div style={{ width: '70%', height: '100%', position: 'relative' }}>
        <GoogleMap mapContainerStyle={containerStyle} center={userLocation || defaultHyderabadLocation} zoom={14} onLoad={onMapLoad} options={{ styles: darkMapStyles, disableDefaultUI: true, zoomControl: true }}>
          {userLocation && (<>
            <Marker position={userLocation} title="Your Location" icon={userMarkerIcon} />
            <Marker position={userLocation} title="Zyntra Mobile Charger" icon={zyntraMarkerIcon} onClick={() => {
              const zyntraStation: Station = { place_id: 'zyntra_user_location', name: 'Zyntra Mobile Charger', geometry: { location: { lat: () => userLocation.lat, lng: () => userLocation.lng } }, formatted_address: 'Your Current Location', rating: 4.9, user_ratings_total: 1200, opening_hours: { isOpen: () => true }, formatted_phone_number: '+91 1800 123 4567' };
              showStationDetails(zyntraStation);
            }} />
          </>)}
          {stations.map((station) => (<Marker key={station.place_id} position={getLocationLiteral(station.geometry.location)} title={station.name} icon={evStationMarkerIcon} onClick={() => showStationDetails(station)} />))}
        ...
        {userLocation && (
          <div style={{ position: 'absolute', top: '20px', right: '20px', left: 'auto', background: 'linear-gradient(135deg, rgba(70, 0, 90, 0.9) 0%, rgba(120, 0, 150, 0.9) 100%)', border: `1px solid #FF00FF`, borderRadius: '15px', padding: '20px', boxShadow: `0 0 30px rgba(255, 0, 255, 0.6), inset 0 0 15px rgba(255, 105, 255, 0.4)`, zIndex: 10, maxWidth: '280px', color: '#FFFFFF', fontFamily: 'Roboto, sans-serif', animation: 'fadeInSlideRight 0.7s ease-out forwards', backdropFilter: 'blur(5px)', WebkitBackdropFilter: 'blur(5px)', borderTop: `3px solid #FF66FF`, }}>
            ...
            <div style={{ position: 'absolute', top: '-10px', right: '-10px', backgroundColor: '#FF00FF', color: 'white', borderRadius: '50%', width: '30px', height: '30px', display: 'flex', justifyContent: 'center', alignItems: 'center', fontSize: '14px', fontWeight: 'bold', boxShadow: `0 0 15px #FF00FF` }}>Z</div>
            <h3 style={{ margin: '0 0 12px 0', color: '#FF66FF', display: 'flex', alignItems: 'center', textShadow: `0 0 10px #FF00FF`, fontSize: '1.3em', fontWeight: 'bold', letterSpacing: '0.5px' }}><FaChargingStation style={{ marginRight: '10px', color: '#FF66FF', filter: 'drop-shadow(0 0 5px #FF00FF)' }} />ZYNTRA STATION</h3>
            <p style={{ margin: '0 0 15px 0', color: 'rgba(255, 255, 255, 0.8)', fontSize: '0.95em', lineHeight: '1.5' }}>Mobile charging unit available at your location. <span style={{ display: 'block', marginTop: '5px', color: '#FF66FF', fontWeight: 'bold' }}>No need to travel - we come to you!</span></p>
            <button onClick={() => { const zyntraStation: Station = { place_id: 'zyntra_user_location', name: 'Zyntra Mobile Charger', geometry: { location: { lat: () => userLocation.lat, lng: () => userLocation.lng } }, formatted_address: 'Your Current Location', rating: 4.9, user_ratings_total: 1200, opening_hours: { isOpen: () => true }, formatted_phone_number: '+91 1800 123 4567' }; handleBookSlot(zyntraStation); }} style={{ padding: '12px 15px', background: 'linear-gradient(90deg, #FF00FF 0%, #CC00FF 100%)', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontSize: '1em', fontWeight: 'bold', transition: 'all 0.3s ease', boxShadow: `0 0 20px rgba(255, 0, 255, 0.7)`, display: 'flex', alignItems: 'center', width: '100%', justifyContent: 'center', textTransform: 'uppercase', letterSpacing: '1px', position: 'relative', overflow: 'hidden' }} onMouseOver={(e) => { e.currentTarget.style.background = 'linear-gradient(90deg, #FF66FF 0%, #DD00FF 100%)'; e.currentTarget.style.boxShadow = `0 0 30px rgba(255, 0, 255, 0.9)`; e.currentTarget.style.transform = 'translateY(-2px)'; }} onMouseOut={(e) => { e.currentTarget.style.background = 'linear-gradient(90deg, #FF00FF 0%, #CC00FF 100%)'; e.currentTarget.style.boxShadow = `0 0 20px rgba(255, 0, 255, 0.7)`; e.currentTarget.style.transform = 'translateY(0)'; }}><FaCalendarAlt style={{ marginRight: '10px', filter: 'drop-shadow(0 0 3px rgba(255,255,255,0.5))' }} /> Book Mobile Unit<span style={{ position: 'absolute', top: '-50%', left: '-50%', width: '200%', height: '200%', background: 'linear-gradient(transparent, rgba(255,255,255,0.1), transparent)', transform: 'rotate(30deg)', pointerEvents: 'none' }}></span></button>
          </div>
        )}
      </div>
      ...
      <div style={{ width: '30%', height: '100vh', padding: '30px', boxSizing: 'border-box', backgroundColor: primaryBg, color: textColor, overflowY: 'auto', borderLeft: `1px solid ${glowingGreen}`, boxShadow: `inset 5px 0 15px rgba(0,0,0,0.5), 0 0 20px ${glowingGreen}33`, }}>
        ...
        {selectedStation ? (<>
          <button onClick={showAllStations} style={{ marginBottom: '25px', padding: '12px 20px', backgroundColor: accentColor, color: primaryBg, border: 'none', borderRadius: '8px', cursor: 'pointer', display: 'flex', alignItems: 'center', fontSize: '1.1em', fontWeight: 'bold', transition: 'all 0.3s ease-in-out', boxShadow: `0 0 15px ${accentColor}88`, width: '100%', justifyContent: 'center', }} onMouseOver={(e) => { e.currentTarget.style.backgroundColor = '#1EE0B9'; e.currentTarget.style.boxShadow = `0 0 25px ${accentColor}`; e.currentTarget.style.transform = 'translateY(-2px)'; }} onMouseOut={(e) => { e.currentTarget.style.backgroundColor = accentColor; e.currentTarget.style.boxShadow = `0 0 15px ${accentColor}88`; e.currentTarget.style.transform = 'translateY(0)'; }}><FaArrowLeft style={{ marginRight: '12px' }} /> Back to All Stations</button>
          <div style={{ marginBottom: '25px', padding: '25px', background: secondaryBg, border: `1px solid ${glowingGreen}44`, borderRadius: '12px', boxShadow: `0 0 20px ${glowingGreen}22`, }}>
            ...
            {selectedStation.formatted_address && (<p style={{ color: mutedTextColor, marginBottom: '12px', fontSize: '1.1em' }}>{selectedStation.formatted_address}</p>)}
            ...
            {selectedStation?.ocmData && (<div style={{ marginTop: '20px', padding: '20px', background: secondaryBg, border: `1px solid ${glowingGreen}44`, borderRadius: '12px', boxShadow: `0 0 20px ${glowingGreen}22`, }}><h4 style={{ color: accentColor, marginTop: 0, marginBottom: '15px' }}>Charging Details</h4>
              {selectedStation.ocmData?.connections && selectedStation.ocmData.connections.length > 0 ? (<div><p style={{ color: textColor, marginBottom: '10px' }}><strong>Connector Types:</strong></p><ul style={{ paddingLeft: '20px', margin: 0 }}>{selectedStation.ocmData.connections.map((conn, i) => (<li key={i} style={{ color: mutedTextColor, marginBottom: '8px' }}>{conn.type} - {conn.power_kw ?? 'N/A'} kW{conn.level && ` (${conn.level})`}{conn.current && ` | ${conn.current}`}</li>))}</ul>{selectedStation.ocmData.pricing && (<p style={{ color: textColor, marginTop: '15px' }}><strong>Pricing:</strong> {selectedStation.ocmData.pricing}</p>)}{selectedStation.ocmData.operator && (<p style={{ color: textColor, marginTop: '10px' }}><strong>Operator:</strong> {selectedStation.ocmData.operator}</p>)}</div>) : (<p style={{ color: mutedTextColor }}>No detailed charging information available</p>)}
            </div>)}
            {selectedStation && (<button onClick={() => handleBookSlot(selectedStation)} style={{ marginTop: '20px', padding: '12px 20px', backgroundColor: glowingGreen, color: primaryBg, border: 'none', borderRadius: '8px', cursor: 'pointer', display: 'flex', alignItems: 'center', fontSize: '1.1em', fontWeight: 'bold', transition: 'all 0.3s ease-in-out', boxShadow: `0 0 15px ${glowingGreen}88`, width: '100%', justifyContent: 'center', }} onMouseOver={(e) => { e.currentTarget.style.backgroundColor = '#1EE0B9'; e.currentTarget.style.boxShadow = `0 0 25px ${glowingGreen}`; e.currentTarget.style.transform = 'translateY(-2px)'; }} onMouseOut={(e) => { e.currentTarget.style.backgroundColor = glowingGreen; e.currentTarget.style.boxShadow = `0 0 15px ${glowingGreen}88`; e.currentTarget.style.transform = 'translateY(0)'; }}><FaCalendarAlt style={{ marginRight: '12px' }} />Book a Slot</button>)}
          </div>
          {directions && directions.routes[0]?.legs[0]?.steps.length > 0 && (<div style={{ marginTop: '25px', fontSize: '1em', lineHeight: '1.7', background: secondaryBg, padding: '20px', borderRadius: '12px', border: `1px solid ${glowingGreen}44`, boxShadow: `0 0 20px ${glowingGreen}22`, }}><h3 style={{ color: accentColor, marginTop: 0, marginBottom: '20px', textShadow: `0 0 10px ${accentColor}66` }}>Turn-by-Turn Directions</h3><ol style={{ paddingLeft: '25px', margin: 0 }}>{directions.routes[0].legs[0].steps.map((step, i) => (<li key={i} style={{ marginBottom: '15px', color: mutedTextColor }}><p dangerouslySetInnerHTML={{ __html: `<strong>${i + 1}.</strong> ${step.instructions}` }} style={{ color: textColor, margin: 0 }} />{step.distance && <span style={{ color: '#90CAF9', fontSize: '0.9em' }}> ({step.distance.text})</span>}</li>))}</ol></div>)}
        </>) : (
          <div id="stationInfo">
            {loading ? <p style={{ color: mutedTextColor, fontSize: '1.1em' }}>Locating EV Charging Stations...</p> : stations.length === 0 ? <p style={{ color: mutedTextColor, fontSize: '1.1em' }}>No EV stations found nearby.</p> : (stations.map((station) => (
              <div key={station.place_id} style={{ marginBottom: '20px', padding: '20px', background: secondaryBg, border: `1px solid ${glowingGreen}33`, borderRadius: '10px', boxShadow: `0 0 15px ${glowingGreen}11`, cursor: 'pointer', transition: 'all 0.3s ease-in-out', }} onMouseOver={(e) => { e.currentTarget.style.backgroundColor = '#333333'; e.currentTarget.style.boxShadow = `0 0 25px ${glowingGreen}44`; e.currentTarget.style.transform = 'translateY(-3px)'; }} onMouseOut={(e) => { e.currentTarget.style.backgroundColor = secondaryBg; e.currentTarget.style.boxShadow = `0 0 15px ${glowingGreen}11`; e.currentTarget.style.transform = 'translateY(0)'; }} onClick={() => showStationDetails(station)}>
                <h4 style={{ color: accentColor, margin: '0 0 10px 0', fontSize: '1.4em' }}>{station.name}</h4>
                {station.formatted_address && (<p style={{ color: mutedTextColor, margin: '0 0 10px 0', fontSize: '1em' }}>{station.formatted_address}</p>)}
                <p style={{ color: textColor, display: 'flex', alignItems: 'center', margin: '0 0 10px 0', fontSize: '1em' }}><FaStar style={{ color: '#FFD700', marginRight: '10px' }} />Rating: {station.rating ? `${station.rating} / 5` : "N/A"}{station.user_ratings_total && ` (${station.user_ratings_total} reviews)`}</p>
              </div>
            )))}
          </div>
        )}
